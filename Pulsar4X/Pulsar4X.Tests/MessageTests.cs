...
namespace Pulsar4X.Tests
{
    ...
    internal class MessageTests
    {
        ...
        public void MessagePumpQueueOperations()
        {
            ...
            Assert.IsTrue(MessageFormatter.TryDeconstructHeader(ref outString, out incomingMessageTypeTest, out authTokenTest));
        ...
        public void ValidHeaderManipulation()
        {
            ...
            var authToken = new AuthenticationToken(Guid.NewGuid(), "hunter2");
            string message = MessageFormatter.GetMessageHeader(messageType, authToken);

            // Decode the header we retrieved above, ensure equality.
            IncomingMessageType testMessageType;
            AuthenticationToken testAuthToken;
            MessageFormatter.TryDeconstructHeader(ref message, out testMessageType, out testAuthToken);
        ...
        public void InvalidHeaderManipulation()
        {
            ...
            var authToken = new AuthenticationToken(Guid.NewGuid(), "hunter2");
            string message;
            Assert.Throws<InvalidEnumArgumentException>(() => message = MessageFormatter.GetMessageHeader(messageType, authToken));
            ...
            AuthenticationToken authTokenTest;
            Assert.IsFalse(MessageFormatter.TryDeconstructHeader(ref message, out messageTypeTest, out authTokenTest));

            // Attempt to get a header for a null authToken.
            Assert.Throws<ArgumentNullException>(() => invalidMsg = MessageFormatter.GetMessageHeader(IncomingMessageType.Echo, null));
            ...
            invalidMsg = "-1;";
            OutgoingMessageType outMessageTypeTest;
            Assert.False(MessageFormatter.TryGetOutgoingMessageType(ref invalidMsg, out outMessageTypeTest));
        }
        ...
        public void GuidListConversion()
        {
            ...
            string guidListString = MessageFormatter.GuidListToString(guidList);

            // Convert string to new GuidList
            List<Guid> retrievedGuids;
            Assert.IsTrue(MessageFormatter.TryGetGuidList(ref guidListString, out retrievedGuids));
            ...
            guidListString += ';';
            Assert.IsFalse(MessageFormatter.TryGetGuidList(ref guidListString, out retrievedGuids));
        }
        ...
        public void GuidConversion()
        {
            ...
            Guid parsedGuid;
            Assert.IsTrue(MessageFormatter.TryGetGuid(ref guidString, out parsedGuid));
            ...
            guidString = $"{guid:B};";
            Assert.IsFalse(MessageFormatter.TryGetGuid(ref guidString, out parsedGuid));
        }
    }
}
